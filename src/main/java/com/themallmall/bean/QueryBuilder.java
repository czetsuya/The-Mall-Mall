...
public class QueryBuilder {
	...
	private Logger log = LoggerFactory.getLogger(QueryBuilder.class);
	...
	public QueryBuilder addCriterionDateTruncatedToDay(String field, Date value) {
		...
		return addSqlCriterion(field + ">=:start", "start", start)
				.addSqlCriterion(field + "<=:end", "end", end);
	...
	public QueryBuilder addInCriterion(String field, Collection<?> values) {
		...
		int nbSublists = (listSize / NB_MAX_IN_CLAUSE)
				+ (listSize % NB_MAX_IN_CLAUSE > 0 ? 1 : 0);
		...
		for (int i = 0; i < nbSublists; i++) {
			...
			addSqlCriterion(field + " in (:" + paramName + ")", paramName,
					sublist);
		}
	...
	public static List<List<?>> getInLists(List<?> values, int size) {
		...
		for (int i = 0; i < nbSublists; i++) {
			results.add(values.subList(i * size,
					Math.min((i + 1) * size, listSize)));
		}
		return results;
	}
	...
	private String buildCountQueryString() {
		...
		if (trimmedSql.toLowerCase().startsWith(SELECT)) {
			String betweenSelectAndFrom = trimmedSql.substring(
					SELECT.length() + 1, iFrom - 1);
